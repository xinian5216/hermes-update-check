...
| `tests/` | 542 offline tests; fake GitHub client in `tests/conftest.py` |
...
```bash
.venv/bin/python -m pytest -q                         # 542 tests, must stay green, offline
...
Currently released: **v1.3.1** (wheel + sdist attached; interactive checks fetch release data live).
